...
Provide a runtime-only local bridge for guarded Reader AI live fixture execution so provider credentials and retrieval seed context can be supplied for bounded developer runs without committing secrets, raw book text, private book identifiers, or runtime file paths.

...
const entityLookupPattern =
  /(.+?)(是谁|是什么|什么地方|什么组织|什么东西|什么意思|指什么)[？?]?.*$/;
const entityMentionPattern =
  /(?:有没有|有没|是否|是不是|有).*(?:(?:关于|有关)[\p{Script=Han}A-Za-z0-9·-]{2,24}.*(?:内容|信息)|(?:提到|提及|出现)[\p{Script=Han}A-Za-z0-9·-]{2,24})/u;
const entityListPattern = /(.+)(成员有哪些|有哪些成员|都有谁|包括谁|有谁|名单)/;
const entityRecallPattern =
  /(我)?(有点)?忘了[\p{Script=Han}A-Za-z0-9·-]{2,40}.*(是谁|是什么|什么东西|什么组织|什么地方|做过什么|发生过哪些|关键事情|怎么回事)/u;
...
const analysisPattern =
  /(为什么|原因|伏笔|暗示|线索|象征|关系|动机|影响|说明什么|意味着什么|怎么看|\bwhy\b|\bhow\b.*\b(affect|influence|matter|mean|shape)\b|\bwhat\b.*\b(mean|suggest|imply)\b)/i;
...
export const classifyReaderQuestion = ({
  ...
  if (entityMentionPattern.test(normalizedQuestion)) return { intent: 'entity_lookup', scope };
  if (entityRecallPattern.test(normalizedQuestion)) return { intent: 'entity_lookup', scope };
